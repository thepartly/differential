//! The model adapter: an agent CLI on the path, prompt on stdin, completion on
//! stdout (ADR 0016; one argv per agent, ADR 0033).
//!
//! `llm` is the port; this is its one implementation, on the pattern of
//! `forge`/`forgeio`. Nothing else in the engine may reach into subprocess
//! machinery: grouping and the pipeline consume `LlmBackend` from `llm`, the
//! application layer builds a `CommandBackend` here and hands it over, and the
//! layering test names this file as an adapter so that the port's file is
//! checked as domain.
//!
